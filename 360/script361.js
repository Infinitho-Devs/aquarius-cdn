...
/* El overlay movil solo tiene el enlace LINEUP, y ese enlace se oculta
   (display:none) en las paginas que no son el home, porque alli no existe
   la seccion #pmf-lineup. Cuando eso pasa el menu queda sin ningun enlace
   visible: el boton hamburguesa abre un panel vacio. menuMovilVacio()
   detecta ese caso para poder apagar el boton en vez de forzarlo a
   mostrarse en el bucle de guardVisibility(). */
...
function apagarHamburguesa(btnEl){
 var ov=document.getElementById('mobileNavOverlay');if(ov&&ov.classList.contains('paco-abierto')){
...
function guardVisibility(){killShoWareNav();forceBodyMount();if(window.innerWidth<=1280){
 ...
 var sinEnlaces=menuMovilVacio();if(sinEnlaces)apagarHamburguesa(btnEl);
 var targets=sinEnlaces?[menu,actions,logoEl]:[menu,actions,btnEl,logoEl];targets.forEach(function(el){if(el){

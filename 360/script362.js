...
/* El menu hamburguesa existe solo para llevar al LINEUP: el overlay no
   tiene ningun otro enlace. Fuera del home no hay seccion #pmf-lineup, asi
   que el boton abriria un panel vacio. La regla es: sin lineup visible, ni
   boton ni overlay. Se comprueba la seccion directamente (no si el enlace
   quedo en display:none) para no depender de que el script de index.html ya
   haya corrido, que es lo que provocaria un parpadeo al cargar. El chequeo
   de enlaces visibles queda como respaldo por si el overlay se quedara sin
   nada por otro motivo. */
function elVisible(el){
 return !!(el&&(el.offsetWidth||el.offsetHeight||(el.getClientRects&&el.getClientRects().length)));}
function menuMovilVacio(){
 var ov=document.getElementById('mobileNavOverlay');if(!ov)return false;
 if(!elVisible(document.getElementById('pmf-lineup')))return true;
 var enlaces=ov.querySelectorAll('a');for(var i=0;i<enlaces.length;i++){
 if(window.getComputedStyle(enlaces[i]).display!=='none')return false;}
 return true;}
function apagarHamburguesa(btnEl){
 var ov=document.getElementById('mobileNavOverlay');if(ov){
 if(ov.classList.contains('paco-abierto')){
 ov.classList.remove('paco-abierto');
 document.body.style.overflow='';}
 ov.style.setProperty('display','none','important');}
 if(!btnEl)return;
 btnEl.classList.remove('paco-activo');
 btnEl.setAttribute('aria-expanded','false');
 btnEl.style.setProperty('display','none','important');}
function encenderHamburguesa(){
 var ov=document.getElementById('mobileNavOverlay');
 if(ov&&ov.style.display==='none')ov.style.removeProperty('display');}
function guardVisibility(){killShoWareNav();forceBodyMount();if(window.innerWidth<=1280){
 ...
 var logoEl=document.querySelector('.paco-menu__header--logo');
 var sinLineup=menuMovilVacio();if(sinLineup)apagarHamburguesa(btnEl);else encenderHamburguesa();
 var targets=sinLineup?[menu,actions,logoEl]:[menu,actions,btnEl,logoEl];targets.forEach(function(el){if(el){
...
function pasada(){vigilarWrapper();guardarRueda();ponerBadgeRapida();convertirCantidadSelect();ponerAvisoRapidaProximamente();ponerAyuda();marcarPanel();}
pasada();
 document.addEventListener('DOMContentLoaded',pasada);
